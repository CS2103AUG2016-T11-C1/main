...
public class TaskListPanelHandle extends GuiHandle {
    ...
    public boolean containsInOrder(int startPosition, ReadOnlyTask... tasks) {
        ...
        for (int i = 0; i < tasks.length; i++) {
            if (!tasksInList.get(startPosition + i).getTaskDetails().taskDetails.equals(tasks[i].getTaskDetails().taskDetails)){
                return false;
            }
        }
    ...
    public TaskCardHandle navigateToTask(String name) {
        ...
        final Optional<ReadOnlyTask> task = getListView().getItems().stream().filter(p -> p.getTaskDetails().taskDetails.equals(name)).findAny();
        if (!task.isPresent()) {
    ...
    public int getTaskIndex(ReadOnlyTask targetTask) {
        ...
        for (int i = 0; i < tasksInList.size(); i++) {
            if(tasksInList.get(i).getTaskDetails().equals(targetTask.getTaskDetails())){
                return i;
            }
        }
        return NOT_FOUND;
